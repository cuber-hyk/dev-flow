...
- `dev-branch` must show status, changed files, and a concise diff summary, then wait for explicit approval before commit, merge, cleanup, or push. Do not print the full diff by default; include it only when requested, small, or needed for line-level review.
