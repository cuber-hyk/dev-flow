# Dev Distill Output Template

```md
Distillation conclusion: <durable update needed/no durable update/blocked>

Updated or recommended updates:
- <CONTEXT.md/docs/features.md/docs/capabilities/*.md/docs/adr/*.md/tests/context-map>
- or None

No durable update needed:
- <item and reason>
- or None

Artifact closeout:
- Plan: archived/deleted/not applicable/blocked; <path or reason>
- Audit: keep active/archived/deleted/not applicable/blocked; <path or reason>
  Open findings: <IDs or none>
  Transferred to plans: <plan paths and finding IDs or none>
  Verification status: <all closed/partially handled/not verified>
- ADR: accepted/archived/deleted/not needed/blocked; <path or reason>

Verification:
- <command/check/result>

Git visibility:
- <status when files changed; otherwise not applicable>

Next step: <recommended next skill or action>
```
